...
import { Building2, ShoppingBag, Mail, ArrowRight, CheckCircle2, AlertTriangle } from "lucide-react";
...
// Real auth: Supabase magic-link / OTP email, no password anywhere. The role
// tab only decides where a brand-new (never-linked) account lands after
// verifying — resolve-account-session (called from /auth/callback) is what
// actually determines whether this email is a buyer, a vendor, or an
// allow-listed platform admin, so picking "Buyer" here can't grant vendor or
// admin access; it's just a landing-page hint for people with no linked
// record yet.
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-md">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Sign in with your work email — no password, we&apos;ll send you a secure link.</CardDescription>
        </CardHeader>
        <CardContent>
          {status === "sent" ? (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="max-w-xs text-sm text-muted">
                We sent a sign-in link to <span className="font-medium text-foreground">{email}</span>. Open it on this device to
                continue.
              </p>
